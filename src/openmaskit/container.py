...
# Label key OpenMaskit applies to every container it starts. Used for the boot-time
# orphan sweep so we never operate on a container we didn't create.
OPENMASKIT_LABEL_KEY = "openmaskit.server_id"

# Reserved prefix for OpenMaskit-generated container names. User-supplied names
# starting with this are rejected to avoid namespace ambiguity.
OPENMASKIT_NAME_PREFIX = "openmaskit-"
...
def validate_user_container_name(name: str) -> str | None:
    ...
    2. Does not use the reserved `openmaskit-` prefix.
    """
    if not name:
        return "Container name cannot be empty"
    if name.startswith(OPENMASKIT_NAME_PREFIX):
        return (
            f"Container name '{name}' uses the reserved '{OPENMASKIT_NAME_PREFIX}' "
...
async def sweep_server_orphans(
    ...
    """Stop every container labeled `openmaskit.server_id=<server_id>`.

    Used at boot and before a target restart to clear crash-orphans without
    touching containers OpenMaskit didn't start. Best-effort: errors are logged,
    never raised, and a failure on one container does not block the others.
    """
    label_filter = f"label={OPENMASKIT_LABEL_KEY}={server_id}"
    try:
        with anyio.fail_after(timeout):
            result = await anyio.run_process(
