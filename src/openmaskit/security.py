"""Security utilities for OpenMaskit: validation and encryption."""
...
class TokenEncryption:
    ...
    _KEY_PATH = Path("~/.openmaskit/.key").expanduser()
    ...
    def _load_key(self) -> bytes:
        ...
        Priority:
        1. OPENMASKIT_ENCRYPTION_KEY environment variable
        2. ~/.openmaskit/.key file
        3. Generate new key
        """
        # Check env var first
        env_key = os.environ.get("OPENMASKIT_ENCRYPTION_KEY")
        if env_key:
            logger.debug("Using encryption key from OPENMASKIT_ENCRYPTION_KEY")
